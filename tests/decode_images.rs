...
fn test_decode_image() {
    ...
    assert!(height == EXPECTED_HEIGHT, "Decoded image height should be 426");
    let mut img_data = [0_u8; EXPECTED_DATA_LEN];
    decoder.read_image_data(&mut img_data)
...
fn test_decode_4x4() {
    ...
    assert!(height == EXPECTED_HEIGHT, "Decoded image height should be 2");
